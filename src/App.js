...
class Notify{
  alterarTamanho(){
      ...
      let notifyText = document.getElementsByClassName("notify-text")[0]

      let qtdCaracteresTexto = notifyText.innerHTML.length

      if(qtdCaracteresTexto <= 356){
          notifyText.style.fontSize = "16px"
      }
      else if(qtdCaracteresTexto > 356 && qtdCaracteresTexto <= 572){

          notifyText.style.fontSize = "12px"

      }else if(qtdCaracteresTexto > 572 && qtdCaracteresTexto < 680){

          notifyText.style.fontSize = "10px"

      }else if(qtdCaracteresTexto > 680){

          notifyText.style.fontSize = "10px"
          document.getElementsByClassName("notify-text")[0].innerHTML = notifyText.innerHTML.slice(0, 679) + '...'
      }
      ...
      {notificacao.length > 0 ? (
        <Carousel showThumbs={false} autoPlay={true} infiniteLoop={true} showIndicators={false} interval={15000} showStatus={false} showArrows={false} width={width*0.76}>
          {notificacao.map(notificacao => (
            <text className='notify-text' key={notificacao.id_atendimento}>{notificacao.descricao_abertura}</text>
            ))}
        </Carousel>) : <text style={{fontsSize: height*0.24}}>Nenhuma notificação</text>}

        {notify.alterarTamanho()}
    </div>
